...
public class TabellHaug<T extends Comparable<T>> {
	...
	private void reparerOpp() {
		...
		int siste = antall - 1;
		int forelderSiste = siste / 2;
		T temp = null;
		
		
		while (siste > 0 && data[forelderSiste].compareTo(data[siste]) > 0) {
			temp = data[forelderSiste];
			data[forelderSiste] = data[siste];
			data[siste] = temp;
			
			siste = forelderSiste;
			forelderSiste = siste / 2;
		}
